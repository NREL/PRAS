@testset "Convolution" begin
    ...
    @test all(LOLE.(result_1a, result_1a.timestamps) .≈
              LOLE{1,1,Hour}.(MeanEstimate.([0.028, 0.271, 0.028, 0.028])))
    @test EUE(result_1a) ≈ EUE{4,1,Hour,MWh}(MeanEstimate(1.59))
    @test all(EUE.(result_1a, result_1a.timestamps) .≈
              EUE{1,1,Hour,MWh}.(MeanEstimate.([0.29, 0.832, 0.29, 0.178])))

    result_1a5, surplus_1a5 =
        assess(TestSystems.singlenode_a_5min, simspec, resultspecs...)

    @test LOLE(result_1a5) ≈
          LOLE{4,5,Minute}(MeanEstimate(TestSystems.singlenode_a_lole))
    @test all(LOLE.(result_1a5, result_1a5.timestamps) .≈
    ...
    @test all(LOLE.(result_3, result_3.timestamps) .≈
              LOLE{1,1,Hour}.(MeanEstimate.([.14707, .40951, .21268, .40951])))
    @test EUE(result_3) ≈ EUE{4,1,Hour,MWh}(MeanEstimate(11.73276))
    @test all(EUE.(result_3, result_3.timestamps) .≈
