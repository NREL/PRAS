...
emptystors1 = Storages{4, 1, Hour, MW, MWh}(
...
emptystors1_5min = Storages{4, 5, Minute, MW, MWh}(
    (empty_str for _ in 1:2)...,
    (empty_int(4) for _ in 1:3)...,
    (empty_float(4) for _ in 1:5)...,
)
...
emptystors2 = Storages{6, 1, Hour, MW, MWh}(
    (empty_str for _ in 1:2)...,
...
interfaces = Interfaces{4, MW}([1, 1, 2], [2, 3, 3], fill(100, 3, 4), fill(100, 3, 4))
...
threenode_lolps_copperplate = [0.14707, 0.40951, 0.21268, 0.40951]
...
regions = Regions{1, MW}(["Region A", "Region B"], reshape([8, 9], 2, 1))
...
emptystors = Storages{1, 1, Hour, MW, MWh}(
    (String[] for _ in 1:2)...,
...
timestamps =
    ZonedDateTime(2020, 1, 1, 0, tz"UTC"):Hour(1):ZonedDateTime(2020, 1, 1, 1, tz"UTC")
...
emptygenstors = GeneratorStorages{2, 1, Hour, MW, MWh}(
    (String[] for _ in 1:2)...,
    (zeros(Int, 0, 2) for _ in 1:3)...,
    (zeros(Float64, 0, 2) for _ in 1:3)...,
    (zeros(Int, 0, 2) for _ in 1:3)...,
    (zeros(Float64, 0, 2) for _ in 1:2)...,
)
...
test3_esurplus_rt = [3.879 6.618087; 0.0 4.914189]
