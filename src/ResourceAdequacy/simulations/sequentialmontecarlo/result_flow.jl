...
function merge!(x::SMCFlowAccumulator, y::SMCFlowAccumulator)
...
function accumulator(sys::SystemModel{N}, ::SequentialMonteCarlo, ::Flow) where {N}
    ...
    return SMCFlowAccumulator(
        flow_interface,
        flow_interfaceperiod,
        flow_interface_currentsim,
    )
end
...
function record!(
    ...
    for (i, (f, b)) in
        enumerate(zip(problem.interface_forward_edges, problem.interface_reverse_edges))
...
function finalize(
    ...
    return FlowResult{N, L, T, P}(
        nsamples,
        Pair.(fromregions, toregions),
        system.timestamps,
        flow_mean,
        flow_interface_std,
        flow_interfaceperiod_std,
    )
end

# FlowSamples

struct SMCFlowSamplesAccumulator <: ResultAccumulator{SequentialMonteCarlo, FlowSamples}
    flow::Array{Int, 3}
end

function merge!(x::SMCFlowSamplesAccumulator, y::SMCFlowSamplesAccumulator)
    x.flow .+= y.flow
    return
end
...
function record!(
    ...
    for (i, (e_f, e_r)) in
        enumerate(zip(problem.interface_forward_edges, problem.interface_reverse_edges))
        acc.flow[i, t, sampleid] = problem.fp.edges[e_f].flow - problem.fp.edges[e_r].flow
    end
