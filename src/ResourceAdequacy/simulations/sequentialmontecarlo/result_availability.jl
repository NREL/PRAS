...
function merge!(x::SMCGenAvailabilityAccumulator, y::SMCGenAvailabilityAccumulator)
    x.available .|= y.available
    return
end

accumulatortype(::SequentialMonteCarlo, ::GeneratorAvailability) =
...
function merge!(x::SMCStorAvailabilityAccumulator, y::SMCStorAvailabilityAccumulator)
    x.available .|= y.available
    return
end

accumulatortype(::SequentialMonteCarlo, ::StorageAvailability) =
    SMCStorAvailabilityAccumulator
...
function merge!(x::SMCGenStorAvailabilityAccumulator, y::SMCGenStorAvailabilityAccumulator)
    x.available .|= y.available
    return
end

accumulatortype(::SequentialMonteCarlo, ::GeneratorStorageAvailability) =
...
function finalize(
    ...
    return GeneratorStorageAvailabilityResult{N, L, T}(
        system.generatorstorages.names,
        system.timestamps,
        acc.available,
    )
end
...
function merge!(x::SMCLineAvailabilityAccumulator, y::SMCLineAvailabilityAccumulator)
    x.available .|= y.available
    return
end
