...
function accumulator(sys::SystemModel{N}, ::SequentialMonteCarlo, ::Utilization) where {N}
    ...
    return SMCUtilizationAccumulator(
        util_interface,
        util_interfaceperiod,
        util_interface_currentsim,
    )
end
...
function finalize(
    ...
    nsamples =
        length(system.interfaces) > 0 ? first(acc.util_interface[1].stats).n : nothing
...
accumulatortype(::SequentialMonteCarlo, ::UtilizationSamples) =
    SMCUtilizationSamplesAccumulator
