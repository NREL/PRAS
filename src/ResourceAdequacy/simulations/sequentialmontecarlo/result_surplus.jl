...
function merge!(x::SMCSurplusAccumulator, y::SMCSurplusAccumulator)
...
function accumulator(sys::SystemModel{N}, ::SequentialMonteCarlo, ::Surplus) where {N}
    nregions = length(sys.regions)

    surplus_period = [meanvariance() for _ in 1:N]
    surplus_regionperiod = [meanvariance() for _ in 1:nregions, _ in 1:N]

    return SMCSurplusAccumulator(surplus_period, surplus_regionperiod)
end
...
function merge!(x::SMCSurplusSamplesAccumulator, y::SMCSurplusSamplesAccumulator)
    x.surplus .+= y.surplus
    return
end
