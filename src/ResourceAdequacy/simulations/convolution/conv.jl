CapacityDistribution = DiscreteNonParametric{Int, Float64, Vector{Int}, Vector{Float64}}
...
function spconv(hvsraw::AbstractVector{Int}, hpsraw::AbstractVector{Float64})
    ...
    length(hvs) == 0 && return DiscreteNonParametric([0], [1.0], check_args=false)
    ...
    for (hv, hp) in zip(hvs[2:end], hps[2:end])
        current_values, current_probs, current_n, prev_values, prev_probs = spconv!(
            prev_values,
            prev_probs,
            hv,
            hp,
...
function spconv!(
    y_values::Vector{Int},
    y_probs::Vector{Float64},
    h_value::Int,
    h_prob::Float64,
    x_values::Vector{Int},
    x_probs::Vector{Float64},
    nx::Int,
)
    h_q = 1 - h_prob
