...
function ELCC{M}(capacity_max::Int, region::String; kwargs...) where {M}
...
function assess(
    ...
    elcc_regions, base_load, sys_variable = copy_load(sys_augmented, params.regions)
...
function update_load!(
    ...
    for (r, share) in region_shares
        sys.regions.load[r, :] .= load_base[r, :] .+ round(Int, share * load_increase)
    end
end
