...
function savemodel(
    ...
    verbose && @info "The PRAS system being exported is of type $(typeof(sys))"
    ...
    verbose &&
        @info "Successfully exported the PRAS SystemModel to " * ".pras file " * outfile

    return
end

function process_metadata!(f::File, sys::SystemModel{N, L, T, P, E}) where {N, L, T, P, E}
    attrs = attributes(f)
    ...
    attrs["pras_dataversion"] = PRAS_VERSION

    return
end

function process_regions!(f::File, sys::SystemModel, strlen::Int, compression::Int)
...
function process_generators!(f::File, sys::SystemModel, strlen::Int, compression::Int)
    ...
    gens_core[:, 3] =
        regionnames(length(sys.generators), sys.regions.names, sys.region_gen_idxs)
...
function process_storages!(f::File, sys::SystemModel, strlen::Int, compression::Int)
    ...
    stors_core[:, 3] =
        regionnames(length(sys.storages), sys.regions.names, sys.region_stor_idxs)

    string_table!(storages, "_core", stors_core_colnames, stors_core, strlen)

    storages["chargecapacity", deflate=compression] = sys.storages.charge_capacity

    storages["dischargecapacity", deflate=compression] = sys.storages.discharge_capacity

    storages["energycapacity", deflate=compression] = sys.storages.energy_capacity

    storages["chargeefficiency", deflate=compression] = sys.storages.charge_efficiency

    storages["dischargeefficiency", deflate=compression] = sys.storages.discharge_efficiency

    storages["carryoverefficiency", deflate=compression] = sys.storages.carryover_efficiency
...
function process_generatorstorages!(
    ...
    genstors_core[:, 3] = regionnames(
        length(sys.generatorstorages),
        sys.regions.names,
        sys.region_genstor_idxs,
    )

    string_table!(generatorstorages, "_core", genstors_core_colnames, genstors_core, strlen)

    generatorstorages["inflow", deflate=compression] = sys.generatorstorages.inflow
    ...
    generatorstorages["failureprobability", deflate=compression] = sys.generatorstorages.λ

    generatorstorages["repairprobability", deflate=compression] = sys.generatorstorages.μ

    return
end

function process_lines_interfaces!(f::File, sys::SystemModel, strlen::Int, compression::Int)
    ...
    for (lines, r_from, r_to) in
        zip(sys.interface_line_idxs, sys.interfaces.regions_from, sys.interfaces.regions_to)
    ...
    lines["forwardcapacity", deflate=compression] = sys.lines.forward_capacity

    lines["backwardcapacity", deflate=compression] = sys.lines.backward_capacity
    ...
    ints_core[:, 1] = getindex.(Ref(sys.regions.names), sys.interfaces.regions_from)
    ints_core[:, 2] = getindex.(Ref(sys.regions.names), sys.interfaces.regions_to)
    string_table!(interfaces, "_core", ints_core_colnames, ints_core, strlen)

    interfaces["forwardcapacity", deflate=compression] = sys.interfaces.limit_forward

    interfaces["backwardcapacity", deflate=compression] = sys.interfaces.limit_backward
...
function regionnames(
    ...
    unit_idxs::Vector{UnitRange{Int}},
)
...
function string_table!(
    ...
    allunique(colnames) || error("All column names must be unique")
    ...
    dset = create_dataset(f, tablename, Datatype(dt_id), dataspace((nrows,)))
    h5d_write(dset, dt_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, rawdata)
end
